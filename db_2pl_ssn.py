...
class NonRR(AbortTransaction):
    '''Read multiple versions of the same record'''
    pass
...
def make_db(tracker, stats, db_size, tid_watch, rid_watch, read_only_opt,
    ...
    def tx_read(pid, rid, for_update=False):
        ...
        When emulating 2PL+SSN, W-W conflicts are already handled for
        us by the tracker. Reads would normally require some effort,
        since they use short duration locks, but we can exploit the
        simulator's non-preemptive scheduler to turn those into
        instant locks instead, with the desirable result that we don't
        have to track read locks for R-W conflicts at all (SSN will do
        that for us). We'll add extra sauce here to model W-R
        conflicts, giving reads priority over writes (a significant
        benefit to the reader, and harmless to the writer because
        reads are instant). Just watch out for deadlocks! The tracker
        enforces limited wait depth, but we have to tell it about
        readers for that to work properly.
        ...
        xmin,_ = yield from tracker.on_access(pid, rid, True)
        ...
        if safe_snapper.get_snap(pid):
            ...
            if rc is None:
                # in-flight reader, have to revisit during pre-commit
