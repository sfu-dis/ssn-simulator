...
# oopsie...
1 or errlog('''python3 main.py --db-model=ssi --nclients=160 --duration=10000 --seed=xEAIcLd7
!!! Database wedged !!!
Transactions: 10936 10965 11031
!!!                 !!!
''')

def make_ssi_db(tracker, stats, nrec, tid_watch=(), rid_watch=(),
...
def test_ssi_db():
    ...
    for test in (test1,test2,test3,test4,test5,test6,test7,test8,test9,
        ...
        db = make_ssi_db(stats=stats, tracker=tracker,nrec=10, ww_blocks=True, verbose=True)
        db.begin_tracking()
        try:
